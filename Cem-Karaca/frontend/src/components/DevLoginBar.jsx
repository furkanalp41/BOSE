import { useEffect, useState } from 'react'
import { FURKAN_AUTH_BASE, clearAuth, getToken, getUser, setToken } from '../lib/auth'

// DevLoginBar — top-of-page banner that lets any BOSE frontend mint a JWT
// against Furkan's auth service (:8080), regardless of the current port.
//
// Renders a "logged in" pill when a token already exists; otherwise shows
// an inline email/password form. Token is stored in a host-scoped cookie
// so every other BOSE Vite app on localhost inherits it automatically.
export default function DevLoginBar() {
  const [token, setLocalToken] = useState(getToken())
  const [user, setUser] = useState(getUser())
  const [email, setEmail] = useState('')
  const [password, setPassword] = useState('')
  const [open, setOpen] = useState(false)
  const [busy, setBusy] = useState(false)
  const [error, setError] = useState(null)

  useEffect(() => {
    const i = setInterval(() => {
      const t = getToken()
      if (t !== token) {
        setLocalToken(t)
        setUser(getUser())
      }
    }, 1000)
    return () => clearInterval(i)
  }, [token])

  const onSubmit = async (e) => {
    e.preventDefault()
    setBusy(true)
    setError(null)
    try {
      const res = await fetch(`${FURKAN_AUTH_BASE}/auth/login`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ email, password }),
      })
      const data = await res.json()
      if (!res.ok || !data.token) {
        throw new Error(data.error || 'Giriş başarısız')
      }
      setToken(data.token, data.user)
      setLocalToken(data.token)
      setUser(data.user)
      setOpen(false)
      window.location.reload()
    } catch (err) {
      setError(err.message)
    } finally {
      setBusy(false)
    }
  }

  const onLogout = () => {
    clearAuth()
    setLocalToken(null)
    setUser(null)
    window.location.reload()
  }

  if (token) {
    return (
      <div className="bg-neon/10 border-b border-neon/30 text-neon text-xs px-4 py-1.5 flex items-center justify-between">
        <span>
          Oturum: <strong className="text-cloud">{user?.email ?? user?.full_name ?? 'aktif'}</strong>
        </span>
        <button onClick={onLogout} className="underline hover:text-glow-neon">çıkış</button>
      </div>
    )
  }

  return (
    <div className="bg-amber/10 border-b border-amber/30 text-amber text-xs px-4 py-1.5">
      <div className="flex items-center justify-between">
        <span>Giriş yapılmadı — istekler 401 dönecek.</span>
        <button onClick={() => setOpen((v) => !v)} className="underline hover:text-cloud">
          {open ? 'kapat' : 'hızlı giriş'}
        </button>
      </div>
      {open && (
        <form onSubmit={onSubmit} className="mt-2 flex flex-wrap gap-2 items-center">
          <input type="email" required placeholder="email" value={email}
            onChange={(e) => setEmail(e.target.value)}
            className="px-2 py-1 bg-white/5 border border-edge rounded text-cloud text-xs outline-none focus:border-neon" />
          <input type="password" required placeholder="şifre" value={password}
            onChange={(e) => setPassword(e.target.value)}
            className="px-2 py-1 bg-white/5 border border-edge rounded text-cloud text-xs outline-none focus:border-neon" />
          <button type="submit" disabled={busy}
            className="px-3 py-1 bg-neon hover:shadow-neon-sm disabled:opacity-50 text-void font-semibold text-xs rounded transition-all">
            {busy ? '...' : 'Giriş Yap'}
          </button>
          <span className="text-silver">hedef: {FURKAN_AUTH_BASE}/auth/login</span>
          {error && <span className="text-crimson">{error}</span>}
        </form>
      )}
    </div>
  )
}
